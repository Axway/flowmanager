...
#### Create volumes for persistence data
 ...
 - mkdir -p ./mounts/fc_keystore     # This folder has to contain either the java keystore (i.e. files with the `.jks` extension) certificates or `.p12` keys.
 - mkdir -p ./mounts/fc_license      # This folder will contain the Flow Central license under the name **license.xml**.
 - mkdir -p ./mounts/fc_api_keys     # This folder will contain the private and public keys required to connect to API Central.
