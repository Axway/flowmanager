...
sed -i "s/mongo:7.0/mongo:8.0/g" ./flowmanager.yml
podman play kube ./flowmanager.yml

echo "Waiting for mongo.."
sleep 30

podman exec -it flowmanager_pod-mongodb bash -c "mongosh -u $ROOT_DB_USER -p $ROOT_DB_PASS --eval \"db.adminCommand( {setFeatureCompatibilityVersion: '8.0', confirm: true} )\""

echo "MongoDB was upgraded to 8.0!"