...
function gen_cert() {
    ...
    openssl req -passin pass:$password -passout pass:$password -batch -newkey rsa:2048 -out $caroot/$name-csr.pem -keyout $caroot/$name-key.pem -subj "/C=FR/O=ACME/CN=$site/OU=ACME-OU/ST=StateUnknown/L=NoWherePuteaux/emailAddress=$email"
...
cp ./custom-ca/governance/cacert.p12 ./configs/governanceca.p12
cp ./custom-ca/business/cacert.p12 ./configs/businessca.p12
cp ./custom-ca/governance/uicert.p12 ./configs/uicert.p12
