...
while [[ RET -ne 0 ]]; do
    ...
    sleep 5
    mongo admin  --port=27018 --eval "help" >/dev/null 2>&1
    RET=$?
done

# Create the admin user
echo "=> Creating admin user with a password in MongoDB"
mongo admin  --port=27018 --eval "db.createUser({user: '$MONGODB_ADMIN_USER', pwd: '$MONGODB_ADMIN_PASS', roles:[{role:'root',db:'admin'}]});"
...
if [ "$MONGODB_APPLICATION_DATABASE" != "admin" ]; then
    ...
    mongo admin  --port=27018 -u $MONGODB_ADMIN_USER -p $MONGODB_ADMIN_PASS << EOF
...
echo ""
echo "    mongo admin  --port=27018 -u $MONGODB_ADMIN_USER -p $MONGODB_ADMIN_PASS --host <host> --port <port>"
echo ""
